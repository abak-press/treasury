...
RSpec.describe Treasury::Lock do
  ...
  describe '#lock' do
    ...
    it { expect(redis.sismember(described_class::KEY, :foo)).to be_truthy }
  end
  ...
  describe '#lock!' do
    ...
    context 'when does not locked' do
      it do
        blocker.lock!
        expect(redis.sismember(described_class::KEY, :foo)).to be_truthy
      end
    end
  end
  ...
  describe '#unlock' do
    it do
      ...
      blocker.unlock
      expect(redis.sismember(described_class::KEY, :foo)).to be_falsy
    end
  end
end
